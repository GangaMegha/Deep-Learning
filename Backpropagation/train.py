...
# Neural Network architecture for arbitrary architecture and hyperparameters
# '''
# --lr 0.01 --momentum 0.5 --num_hidden 3 --sizes 100,100,100
...
# TO DO
# -Implement a solver with the following controls:
# 	-lr:learning rate
# 	-momentum
# 	-batch size
# 	-anneal: true/false
...
class MultiLayer_NeuralNet():
	...
	'''
	def solve_network(self,X,y,X_val,y_val):
		
		if self.opt=='nag':
			self.nag(X,y,X_val,y_val)
			return
...
def get_data(arg_val):
	...
	config = vars(arg_val)
  	...
  	if config['pretrain'] == 'false':
		x_train = np.genfromtxt("scale_train.csv", delimiter=',', missing_values="NaN", skip_header=-1).astype("float")
		x_val = np.genfromtxt("scale_val.csv", delimiter=',', missing_values=".NaN", skip_header=-1).astype("float")

		y_train  = np.eye(10)[np.array(x_train[:, -1]).astype("int")]
		y_val = np.eye(10)[np.array(x_val[:, -1]).astype("int")]

		x_train  = x_train[:, 1:-1]
		x_val = x_val[:, 1:-1]
		config.update({"input_layer_size" : x_train.shape[1], "output_layer_size" : 10})
	...
	if config["pretrain"]=='false':
		...
		nn.solve_network(x_train, y_train, x_val, y_val)
  
	# Predict for test data
	nn.feed_forward(x_test)  
	y_pred = np.argmax(nn.h[-1], axis=1)

	data = np.column_stack((np.arange(len(y_pred)), y_pred))

	np.savetxt("expt_dir/test_submission_adam.csv".format(config["sizes"][0]),  data,delimiter=',', comments='', newline='\n', header="id,label",fmt='%d')
	print("DONE")
...
def parse():
	...
	parser.add_argument('--train', action="store", dest="train", default="scale_train.csv", type = str)
	parser.add_argument('--test', action="store", dest="test", default="scale_test.csv", type = str)
